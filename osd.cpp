...
2012-02-09 - Split character rom out to separate header file, with upper 128
entries as rotated copies of the first 128 entries.  -- AMR
...
void OsdSetSize(int n) { osd_size = n; }

int OsdGetSize() { return osd_size; }
...
void StarsUpdate() {
  ...
  for (int i = 0; i < 64; ++i) {
    ...
    if ((stars[i].x < 0) || (stars[i].x > (228 << 4)) || (stars[i].y < 0) ||
        (stars[i].y > (128 << 4))) {
      stars[i].x = 228 << 4;
...
void OsdSetArrow(int a) { arrow = a; }

void OsdWrite(unsigned char n, const char *s, unsigned char invert,
              unsigned char stipple, char usebg, int maxinv, int mininv) {
...
void OsdWriteOffset(unsigned char n, const char *s, unsigned char invert,
                    unsigned char stipple, char offset, char leftchar,
                    char usebg, int maxinv, int mininv) {
  ...
  while (1) {
    ...
    if (i == 0 && (n < osd_size)) { // Render sidestripe
      unsigned char tmp[8];
      ...
      draw_title(p);
      i += 22;
    } else if (n == (osd_size - 1) &&
               (arrowmask & OSD_ARROW_LEFT)) { // Draw initial arrow
      unsigned char b;
      ...
      p = &charfont[0x10][0];
      for (b = 0; b < 8; b++)
        osdbuf[osdbufpos++] = (*p++ << offset) ^ xormask;
      p = &charfont[0x14][0];
      for (b = 0; b < 8; b++)
        osdbuf[osdbufpos++] = (*p++ << offset) ^ xormask;
      osdbuf[osdbufpos++] = xormask;
      ...
      if (*s++ == 0)
        break; // Skip 3 characters, to keep alignent the same.
      if (*s++ == 0)
        break;
      if (*s++ == 0)
        break;
      ...
      if (b == 0xb) {
        ...
        xorchar ^= 0xff;
      } else if (b == 0x0d || b == 0x0a) { // cariage return / linefeed, go to
                                           // next line increment line counter
        if (++n >= linelimit)
          n = 0;
        ...
        for (c = 0; c < 8; c++) {
          ...
          osdbuf[osdbufpos++] =
              (((*p++ << offset) & stipplemask) ^ xormask ^ xorchar) | bg;
          stipplemask ^= stipple;
        }
        i += 8;
      }
  ...
  if (n == (osd_size - 1) &&
      (arrowmask & OSD_ARROW_RIGHT)) { // Draw final arrow if needed
    unsigned char c;
    osdbuf[osdbufpos++] = xormask;
    osdbuf[osdbufpos++] = xormask;
    osdbuf[osdbufpos++] = xormask;
...
void OsdDrawLogo(int row) {
  ...
  if ((uint)row >= (sizeof(logodata) / sizeof(logodata[0])))
    lp = 0;
...
// write a null-terminated string <s> to the OSD buffer starting at line <n>
static void print_line(unsigned char line, const char *hdr, const char *text,
  ...
  // offset : scroll offset in pixels counting from the start of the string
  // (0-7) invert : invertion flag
...
void ScrollText(char n, const char *str, int off, int len, int max_len,
                unsigned char invert, int idx) {
  // this function is called periodically when a string longer than the window
  // is displayed.

#define BLANKSPACE                                                             \
  10 // number of spaces between the end and start of repeated name
  ...
  if (str && str[0] &&
      CheckTimer(
          scroll_timer[idx])) // scroll if long name and timer delay elapsed
  {
    hdr[0] = 0;
    if (off) {
      strncpy(hdr, str, off);
    ...
    scroll_timer[idx] =
        GetTimer(SCROLL_DELAY2); // reset scroll timer to repeat delay
    ...
    if (off + 2 + len > max_len) // scroll name if longer than display size
    {
      ...
      if (scroll_offset[idx] >= (uint)(len + BLANKSPACE) << 3)
        scroll_offset[idx] = 0;

      offset = scroll_offset[idx] >>
               3; // get new starting character of the name (scroll_offset is no
                  // longer in 2 pixel unit)
      ...
      if (len < max_len - BLANKSPACE) // file name substring and blank space is
                                      // shorter than display line size
      {
        strncpy(s + len + BLANKSPACE, str,
                max_len - len -
                    BLANKSPACE); // repeat the name after its end and predefined
                                 // number of blank space
      }

      print_line(n, hdr, s, (max_len - 1) << 3, (scroll_offset[idx] & 0x7),
                 invert); // OSD print function with pixel precision
    }
  }
}
...
void ScrollReset(int idx) {
  scroll_timer[idx] =
      GetTimer(SCROLL_DELAY); // set timer to start name scrolling after
...
void OsdCoreNameSet(const char *str) { sprintf(lastcorename, "%s", str); }

char *OsdCoreNameGet() { return lastcorename; }
