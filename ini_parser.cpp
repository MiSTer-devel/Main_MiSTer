...
void ini_parse(const ini_cfg_t* cfg)
{
	...
	if (!FileOpen(&ini_file, cfg->filename))
	{
		...
		else ini_parser_debugf("Opened file %s with size %" PRIu64 " bytes.", cfg->filename_alt, ini_file.size);
	}
	else ini_parser_debugf("Opened file %s with size %" PRIu64 " bytes.", cfg->filename, ini_file.size);
