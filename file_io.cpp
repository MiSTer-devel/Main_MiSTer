...
{
	// If the name ends with .bmp then don't modify it
	...
	else
	{
		...
		if (tm.tm_year >= 119) // 2019 or up considered valid time
		{
			...
			snprintf(out_name, buflen, "%s/%s/%s-%s.bmp", SCREENSHOT_DIR, CoreName2, datecode, name[0] ? name : SCREENSHOT_DEFAULT);
		}
		else
		{
			for (int i = 1; i < 10000; i++)
			{
				snprintf(out_name, buflen, "%s/%s/NODATE-%s_%04d.bmp", SCREENSHOT_DIR, CoreName2, name[0] ? name : SCREENSHOT_DEFAULT, i);
