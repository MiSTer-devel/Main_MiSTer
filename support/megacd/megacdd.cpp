...
int cdd_t::LoadCUE(const char *filename) {
  ...
  while (sgets(line, sizeof(line), &buf)) {
    ...
    if (!(memcmp(lptr, "FILE", 4))) {
      ...
      if (*lptr == '\"') {
        lptr++;
        while ((*lptr != '\"') && (lptr <= (line + 128)) &&
               (ptr < (fname + 1023)))
          *ptr++ = *lptr++;
      } else {
        while ((*lptr != 0x20) && (lptr <= (line + 128)) &&
               (ptr < (fname + 1023)))
          *ptr++ = *lptr++;
      }
      *ptr = 0;

      if (!FileOpen(&this->toc.tracks[this->toc.last].f, fname))
        return -1;
    ...
    else if ((sscanf(lptr, "TRACK %02d %*s", &bb)) ||
             (sscanf(lptr, "TRACK %d %*s", &bb))) {
    ...
    /* decode INDEX commands */
      ...
      if (this->toc.last && !this->toc.tracks[this->toc.last - 1].end) {
        this->toc.tracks[this->toc.last - 1].end =
            bb + ss * 75 + mm * 60 * 75 + pregap;
      }
      ...
      if (!this->toc.tracks[this->toc.last].f.opened()) {
        ...
        this->toc.tracks[this->toc.last].start =
            bb + ss * 75 + mm * 60 * 75 + pregap;
        if (this->toc.last && !this->toc.tracks[this->toc.last - 1].end) {
          this->toc.tracks[this->toc.last - 1].end =
              this->toc.tracks[this->toc.last].start;
        ...
        if (this->toc.tracks[this->toc.last].type)
          sectorSize = this->toc.tracks[0].sector_size;
        this->toc.tracks[this->toc.last].end =
            this->toc.tracks[this->toc.last].start +
            ((this->toc.tracks[this->toc.last].f.size + sectorSize - 1) /
             sectorSize);
      ...
      printf("\x1b[32mMCD: Track = %u, start = %u, end = %u, offset = %u, type "
             "= %u\n\x1b[0m",
             cdd.toc.last, cdd.toc.tracks[cdd.toc.last].start,
             cdd.toc.tracks[cdd.toc.last].end,
             cdd.toc.tracks[cdd.toc.last].offset,
             cdd.toc.tracks[cdd.toc.last].type);
...
int cdd_t::Load(const char *filename) {
  ...
  if ((getCDROMType(0) == DISC_MEGACD || getCDROMType(0) == DISC_UNKNOWN) &&
    ...
    if (count > 0) {
      // Mimic Reset() behavior to ensure clean state for new disc checks (e.g.
      // isData)
      this->isData = 1;
      this->lba = 0;
      this->index = 0;
  ...
  if (this->toc.chd_f) {
    mister_chd_read_sector(this->toc.chd_f, 0, 0, 0, 0x10, (uint8_t *)header,
                           this->chd_hunkbuf, &this->chd_hunknum);
  } else {
  ...
  printf("\x1b[32mMCD: Sector size = %u, Track 0 end = %u\n\x1b[0m",
         this->sectorSize, this->toc.tracks[0].end);
...
void cdd_t::Unload() {
  if (this->loaded) {
    ...
    if (this->toc.sub.opened())
      FileClose(&this->toc.sub);
...
void cdd_t::Update() {
  if (this->status == CD_STAT_STOP || this->status == CD_STAT_TRAY ||
    ...
    if (this->lba >= this->toc.tracks[this->index].end) {
      ...
      if (this->toc.tracks[this->index].f.opened()) {
        FileSeek(&this->toc.tracks[this->index].f,
                 (this->toc.tracks[this->index].start * 2352) -
                     this->toc.tracks[this->index].offset,
                 SEEK_SET);
      }
    }
    ...
    if (this->toc.sub.opened())
      FileSeek(&this->toc.sub, this->lba * 96, SEEK_SET);
    ...
    if (this->toc.tracks[this->index].type) {
      ...
      // AUDIO track
      FileSeek(&this->toc.tracks[this->index].f,
               (this->lba * 2352) - this->toc.tracks[this->index].offset,
               SEEK_SET);
    }
  }
}
...
void cdd_t::CommandExec() {
  ...
  case CD_COMM_IDLE:
    if (this->latency <= 3) {
      ...
      if (stat[1] == 0x0f) {
        ...
        stat[2] =
            (cdd.index < this->toc.last) ? BCD(this->index + 1) >> 4 : 0xA;
        stat[3] =
            (cdd.index < this->toc.last) ? BCD(this->index + 1) & 0xF : 0xA;
      }
    }
  ...
  case CD_COMM_TOC:
    ...
    case 0: {
      ...
      // printf("\x1b[32mMCD: Command TOC 0, lba = %i, command =
      // %02X%02X%02X%02X%02X%02X%02X%02X%02X%02X, status = %02X%08X, frame =
      // %u\n\x1b[0m", lba, comm[9], comm[8], comm[7], comm[6], comm[5],
      // comm[4], comm[3], comm[2], comm[1], comm[0], (uint32_t)(GetStatus() >>
      // 32), (uint32_t)GetStatus(), frame);
    } break;
    ...
    case 1: {
      ...
      // printf("\x1b[32mMCD: Command TOC 1, lba = %i, command =
      // %02X%02X%02X%02X%02X%02X%02X%02X%02X%02X, status = %02X%08X, frame =
      // %u\n\x1b[0m", lba, comm[9], comm[8], comm[7], comm[6], comm[5],
      // comm[4], comm[3], comm[2], comm[1], comm[0], (uint32_t)(GetStatus() >>
      // 32), (uint32_t)GetStatus(), frame);
    ...
    case 2: {
      ...
      stat[1] = 0x2;
      stat[2] =
          ((this->index < this->toc.last) ? BCD(this->index + 1) >> 4 : 0xA);
      stat[3] =
          ((this->index < this->toc.last) ? BCD(this->index + 1) & 0xF : 0xA);
      stat[4] = 0;
      stat[5] = 0;
      stat[6] = 0;
      stat[7] = 0;
      ...
      // printf("\x1b[32mMCD: Command TOC 2, index = %i, command =
      // %02X%02X%02X%02X%02X%02X%02X%02X%02X%02X, status = %02X%08X, frame =
      // %u\n\x1b[0m", this->index, comm[9], comm[8], comm[7], comm[6], comm[5],
      // comm[4], comm[3], comm[2], comm[1], comm[0], (uint32_t)(GetStatus() >>
      // 32), (uint32_t)GetStatus(), frame);
    ...
    case 3: {
      ...
      // printf("\x1b[32mMCD: Command TOC 3, lba = %i, command =
      // %02X%02X%02X%02X%02X%02X%02X%02X%02X%02X, frame = %u\n\x1b[0m", lba,
      // comm[9], comm[8], comm[7], comm[6], comm[5], comm[4], comm[3], comm[2],
      // comm[1], comm[0], frame);
    } break;
    ...
    case 4: {
      ...
      // printf("\x1b[32mMCD: Command TOC 4, last = %i, command =
      // %02X%02X%02X%02X%02X%02X%02X%02X%02X%02X, frame = %u\n\x1b[0m",
      // this->toc.last, comm[9], comm[8], comm[7], comm[6], comm[5], comm[4],
      // comm[3], comm[2], comm[1], comm[0], frame);
    } break;
    ...
    case 5: {
      ...
      // printf("\x1b[32mMCD: Command TOC 5, lba = %i, track = %i, command =
      // %02X%02X%02X%02X%02X%02X%02X%02X%02X%02X, frame = %u\n\x1b[0m", lba_,
      // track, comm[9], comm[8], comm[7], comm[6], comm[5], comm[4], comm[3],
      // comm[2], comm[1], comm[0], frame);
    } break;
  ...
  case CD_COMM_PLAY: {
    int lba_;
    MSFToLBA(&lba_, comm[2] * 10 + comm[3], comm[4] * 10 + comm[5],
             comm[6] * 10 + comm[7]);
    lba_ -= 150;
    ...
    // printf("\x1b[32mMCD: Command PLAY, lba = %i, index = %i, command =
    // %02X%02X%02X%02X%02X%02X%02X%02X%02X%02X, frame = %u\n\x1b[0m", lba_,
    // this->index, comm[9], comm[8], comm[7], comm[6], comm[5], comm[4],
    // comm[3], comm[2], comm[1], comm[0], frame);
  } break;
  ...
  case CD_COMM_TRAY_CLOSE:
    ...
    // printf("\x1b[32mMCD: Command TRAY_CLOSE, status = %u, frame =
  ...
  case CD_COMM_TRAY_OPEN:
    ...
    // printf("\x1b[32mMCD: Command TRAY_OPEN, status = %u, frame =
    // %u\n\x1b[0m", this->status, frame);
    break;
  ...
  default:
    ...
    // printf("\x1b[32mMCD: Command undefined, status = %u, frame =
    // %u\n\x1b[0m", this->status, frame);
    break;
  }
}
...
uint64_t cdd_t::GetStatus(uint8_t crc_start) {
  uint8_t n9 = ~(crc_start + stat[0] + stat[1] + stat[2] + stat[3] + stat[4] +
                 stat[5] + stat[6] + stat[7] + stat[8]);
  return ((uint64_t)(n9 & 0xF) << 36) | ((uint64_t)(stat[8] & 0xF) << 32) |
         ((uint64_t)(stat[7] & 0xF) << 28) | ((uint64_t)(stat[6] & 0xF) << 24) |
         ((uint64_t)(stat[5] & 0xF) << 20) | ((uint64_t)(stat[4] & 0xF) << 16) |
         ((uint64_t)(stat[3] & 0xF) << 12) | ((uint64_t)(stat[2] & 0xF) << 8) |
         ((uint64_t)(stat[1] & 0xF) << 4) | ((uint64_t)(stat[0] & 0xF) << 0);
}
...
int cdd_t::SetCommand(uint64_t c, uint8_t crc_start) {
  ...
  uint8_t crc = (~(crc_start + comm[0] + comm[1] + comm[2] + comm[3] + comm[4] +
...
void cdd_t::SeekToLBA(int lba, int play) {
  ...
  while ((this->toc.tracks[index].end <= lba) && (index < this->toc.last))
    index++;
  this->index = index;
  ...
  if (this->toc.tracks[index].type) {
    ...
    /* PCM AUDIO track */
    FileSeek(&this->toc.tracks[index].f,
             (lba * 2352) - this->toc.tracks[index].offset, SEEK_SET);
  }
  ...
  if (this->toc.sub.opened())
    FileSeek(&this->toc.sub, lba * 96, SEEK_SET);
}
...
void cdd_t::ReadData(uint8_t *buf) {
  if (this->toc.tracks[this->index].type && (this->lba >= 0)) {
    ...
    if (cd_type == DISC_MEGACD && hasCDROMMedia(0)) {
      // Try reading from physical CD with retries
      // If read fails (e.g. during seek), returning uninitialized buffer causes
      // BIOS to see garbage and incorrectly identify the disc (e.g. as Audio
      // CD), jumping to CD Player.
      int retries = 3;
      while (retries--) {
        if (read_cdrom_sector(0, this->lba, buf, 2048) > 0) {
          if (this->lba == 16) {
            printf("\n*** BIOS CHECKING SECTOR 16 (Header) *** SUCCESS\n");
            DebugLog("[CD] READ SECTOR 16 (Header) -> SUCCESS\n");
          }
          return;
        }
        if (this->lba == 16) {
          printf("\n*** BIOS CHECKING SECTOR 16 (Header) *** FAIL - "
                 "RETRYING...\n");
          DebugLog("[CD] READ SECTOR 16 (Header) -> FAIL (Retrying...)\n");
        }
        usleep(2000); // Wait 2ms before retry
      }
      // If all retries fail, clear buffer to avoid garbage data
      if (this->lba == 16) {
        printf("\n*** BIOS CHECKING SECTOR 16 (Header) *** FAILED ALL RETRIES "
               "- SENDING ZEROS\n");
        DebugLog("[CD] READ SECTOR 16 (Header) -> FAILED ALL RETRIES (Zeroed "
                 "Buffer)\n");
      }
      memset(buf, 0, 2048);
      return;
    ...
    if (this->toc.chd_f) {
      ...
      mister_chd_read_sector(
          this->toc.chd_f, this->lba + this->toc.tracks[0].offset, 0,
          read_offset, 2048, buf, this->chd_hunkbuf, &this->chd_hunknum);
    } else {
      if (this->sectorSize == 2048) {
...
int cdd_t::ReadCDDA(uint8_t *buf) {
  ...
  // printf("\x1b[32mMCD: AUDIO LENGTH %d LBA: %d INDEX: %d START: %d END
  // %d\n\x1b[0m", this->audioLength, this->lba, this->index,
  // this->toc.tracks[this->index].start, this->toc.tracks[this->index].end);
  //
  ...
  if ((cd_type == DISC_MEGACD || cd_type == DISC_UNKNOWN) && hasCDROMMedia(0)) {
    int read_len =
        read_cdrom_sector(0, this->chd_audio_read_lba, buf, this->audioLength);
    if (read_len > 0) {
  ...
  if (this->toc.chd_f) {
    for (int i = 0; i < this->audioLength / 2352; i++) {
      mister_chd_read_sector(
          this->toc.chd_f,
          this->chd_audio_read_lba + this->toc.tracks[this->index].offset,
          2352 * i, 0, 2352, buf, this->chd_hunkbuf, &this->chd_hunknum);
    }
...
int cdd_t::ReadSubcode(uint16_t *buf) {
  ...
  uint8_t subc[96];
  if (this->toc.chd_f) {
    // Just use the read sector call with an offset, since we previously read
    // that sector, it is already in the hunk cache
    if (this->toc.tracks[this->index].sbc_type == SUBCODE_RW_RAW) {
      mister_chd_read_sector(this->toc.chd_f,
                             this->chd_audio_read_lba +
                                 this->toc.tracks[this->index].offset,
                             0, CD_MAX_SECTOR_DATA, 96, (uint8_t *)buf,
                             this->chd_hunkbuf, &this->chd_hunknum);
    } else if (this->toc.tracks[this->index].sbc_type == SUBCODE_RW) {
      mister_chd_read_sector(
          this->toc.chd_f,
          this->chd_audio_read_lba + this->toc.tracks[this->index].offset, 0,
          CD_MAX_SECTOR_DATA, 96, subc, this->chd_hunkbuf, &this->chd_hunknum);
